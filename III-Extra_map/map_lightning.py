...
# -*- coding: utf-8 -*-
"""
Title: Generate Maps of Lightning Strike Density from ERA5 Data Canada and Québec
Usage: This script processes ERA5 data to generate maps showing the density of lightning strikes 
across specific regions, with options for custom visualization settings.
Authors: Mathias Ponton
Created on 2024/05/21

Description:
This script processes ERA5 reanalysis data and applies geographic masks to create visualizations of 
lightning strike density. The script uses Cartopy and Matplotlib to generate maps, applying custom 
colormaps, grids, and colorbars based on the input data.
...
How to use:
The main functions in this script allow you to load and process data, apply geographic masks, and generate maps 
that display the density of lightning strikes or other meteorological data. The `create_map` function can be used 
to customize the appearance of the maps, including the color scheme, geographic extent, and title.

Example:
mask = mask_data('/mask.nc4', 'bdns1')
ds = subset_data('/ds.nc', 'bdns1')
lon, lat, masked_data = final_dataset_era5(mask, ds, 'sum')

create_map(lon, lat, masked_data, 'density', 0, 5, 0.5, 0.5,
               'Density of strikes on km², May to October', output_path='density_canada_20102019.pdf')
"""
...
#######################################################################
def create_map(lon, lat, masked_data, option, data_min, data_max, data_increment, colorbar_increment, units,
    ...
    ax.text(0.05, 0.95, 'Period: 2015-2023', transform=ax.transAxes, fontsize=15,
