...
# -*- coding: utf-8 -*-
"""
Title: Generate F1 Score Maps for Meteorological Variables
Usage: Compute and map F1 scores across multiple meteorological variables
Authors: Jonathan Durand and Mathias Ponton
Created on 2024/04/01

Description:
This script calculates and generates F1 score maps for various meteorological 
variables including CAPE, CP, CAPE x CP, Kx, and TTX. The F1 scores are computed 
for a range of thresholds and are stored in NetCDF files. The script loops through 
each variable, calculates precision, recall, and F1 score, and saves the results 
...
How to use:
The script iterates over the specified meteorological variables, computes the F1 
...
Example:
The script will save F1 score datasets for each variable at the specified 
output path.
"""
...
path_out = '/path_out/best_f1score_step4/'
...
for var in variables:
    ...
    # Initialize a data structure for F1 scores with a new dimension for thresholds
    f1_scores = np.full((len(ds_all.latitude), len(ds_all.longitude), len(seuil)), np.nan)
    ...
    data_var = xr.Dataset({
        f"{var}_f1": (("latitude", "longitude", "threshold"), f1_scores)
    }, coords={
...
for var, data in datasets.items():
    ...
    data.to_netcdf(f"{path_out}{var}_all_f1_scores.nc4", mode='w', format='NETCDF4',
